...
public class xLuaTest : MonoBehaviour {
    ...
    public void TestEncryptionScript()
    {
        ...
        TextAsset file = (TextAsset)Resources.Load("Encrypted.lua");
        string encryptedString = file.text;
        byte[] encryptedBytes = Convert.FromBase64String( encryptedString );
        byte[] decryptedBytes = _encryptionHelper.Decrypt(encryptedBytes, 102 );
        string decryptedString = Encoding.UTF8.GetString(decryptedBytes);
        //Debug.Log( decryptedString );
        _luaEnv.DoString(decryptedString);
