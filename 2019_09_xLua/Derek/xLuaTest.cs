...
public class xLuaTest : MonoBehaviour {
    ...
    public void TestEncryptionScript()
    {
        ...
        string encrpyted = (Resources.Load("Encrypted.lua") as TextAsset)?.text;
        
        byte[] decrypted = _encryptionHelper.Decrypt(Convert.FromBase64String(encrpyted), 102);
        string script = Encoding.Default.GetString(decrypted);
